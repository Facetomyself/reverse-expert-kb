...
- Primary role: gateway / relay / Hysteria + custom DERP host
- SSH alias: `oracle-gateway`
- Main purpose: 当前承担 Hysteria 网关 + 自建 Tailscale DERP 角色，并保留一个临时分发端点与本地上传残留；历史上的 docker/registry proxy 职能已退役，旧 registry/Harbor 残留也已按用户要求永久删除
