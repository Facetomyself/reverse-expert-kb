...
## Summary
`oracle-newapi-standby` is the standby New API / AI API relay host.

Former identity: `oracle-reverse-dev`.

## Active project: New API standby
...
- Role marker header: `X-NewAPI-Role: standby`
- Planned DNS: `newapi-standby.zhangxuemin.work`
- Temporary/direct access before DNS/TLS finalization: `http://140.245.61.236/`
...
The former `aBaiAutoplus` and `GPT-FULL-REGIST-AND-PAYMENT-FLOW` runtime/source payloads were retired on 2026-06-08 before this host was re-positioned as New API standby. See `CHANGELOG.md` for details.
