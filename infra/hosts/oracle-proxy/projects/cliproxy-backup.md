...
- Image: `eceasy/cli-proxy-api:v7.1.56`
- Update helper: `/root/update_cliproxy_backup.sh`
- Update log: `/var/log/cliproxy-backup-update.log`
...
```cron
17 4 * * * /root/update_cliproxy_backup.sh >> /var/log/cliproxy-backup-update.log 2>&1
```

The update helper pulls the configured CLIProxy image, compares image IDs, recreates only when the image changes unless forced, and rolls back to the previous image if startup or health checks fail. The script now defaults to a pinned production image `eceasy/cli-proxy-api:v7.1.56` via `IMAGE_DEFAULT`, while still allowing a temporary override with `CLIPROXY_BACKUP_IMAGE=... /root/update_cliproxy_backup.sh`.

Manual update:

```bash
ssh oracle-proxy
/root/update_cliproxy_backup.sh
```
...
```bash
ssh oracle-proxy
/root/update_cliproxy_backup.sh --force-recreate
```
