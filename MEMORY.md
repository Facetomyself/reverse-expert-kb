...
<!-- openclaw-memory-promotion:memory:memory/2026-04-04.md:10:14 -->
- - 2026-04-04: `oracle-gateway` current live network split was confirmed and should be treated as source-of-truth: `derper` owns public TCP `80/443` for `derp.zhangxuemin.work`, `hysteria` owns UDP `443`, and host `caddy` was intentionally displaced to local/helper ports `8080/8443` for legacy `backup.zhangxuemin.work` handling. `backup` is no longer the primary public HTTPS front door. - 2026-04-04: Cleaned stale `/tmp-upload` Caddy config residue on `oracle-gateway` while keeping DERP as the primary ingress. Docs under `infra/hosts/oracle-gateway/` were aligned to the DERP-era topology. - 2026-04-04: Cloudflare DNS cleanup advanced in two waves. Durable end state: removed stale mail auto-discovery CNAME/SRV/TLSA records, removed duplicate lower-value DMARC and DKIM records, removed `pend.zhangxuemin.work`, and documented the remaining live zone as a grouped 21-record baseline under `infra/cloudflare-dns/`. - 2026-04-04: Workspace Git hygiene improved. `ops-assistant` now ignores transient runtime artifacts (`state/*`, transient logs/stdout/stderr); root `.gitignore` was expanded for scratch/accident files; obviously bogus here-doc/shell-fragment files were moved into `_tmp/workspace-garbage-2026-04-04/` instead of hard-deleting. - 2026-04-04: `infra/` is operating as a nested/independent Git repo; main workspace now ignores nested `infra/.git` metadata to reduce noise. `infra` docs were committed in the `infra` repo separately. [score=0.880 recalls=4 avg=0.726 source=memory/2026-04-04.md:10-14]
