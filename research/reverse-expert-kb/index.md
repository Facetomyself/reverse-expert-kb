...
- **now materially established and worth preserving canonically**:
  ...
  - malware practical workflows, now including a sharper Run-key / StartupApproved continuation around startup-live truth vs artifact truth, a sharper Scheduled Task continuation around live-scheduler / reload truth plus settings-level **conditions truth** when idle / battery / network / delayed-start semantics decide whether present-day no-run should count against the task at all, explicit handoff rules for `ComHandler` and PowerShell Scheduled Job cases, and a thinner dedicated continuation once the real proof object lives in one ScheduledJobs-store definition rather than in task inventory alone; that Scheduled Job continuation should now preserve the split between **definition truth**, **conditions truth**, and **history truth** so current no-run behavior is not overread; the service-failure / timeout-abuse continuation should now also preserve **launch-context truth** when `SC_ACTION_RUN_COMMAND` is the selected recovery slot, so the branch does not stop at merely proving a configured command or generic child appearance before freezing the service-account-shaped launch context that actually owns the later effect; the Linux persistence side of the malware branch now also has explicit thinner continuations for systemd service/timer/socket activation, cron/anacron dispatch, and `udev` rule -> device-trigger consumer proof so service/unit artifacts, scheduler/job artifacts, and device-triggered rule artifacts do not all collapse back into one vague persistence story; and the comms send-boundary continuation should now preserve a thinner async-wrapper stop rule so WinHTTP **send initiation**, **send-complete / request-sent**, and **headers-available / response** truth stay separate, WinINet **callback installation / callback-eligible context** stays separate from **request-complete truth**, and BITS-style durable transfer ownership still stays separate from **`Complete()` / final-availability** truth rather than collapsing into one vague "network happened" claim; the compact branch-memory shorthand for that seam is now **`initiated != committed != progressed != available`**, so initiation, callback/state progression, and durable availability do not silently collapse back together during practical case work
  - runtime-evidence practical workflows, now including a sharper compare-run continuation around noisy early mismatches where scheduler/timing/randomness churn can mask the first behavior-bearing divergence, plus a clearer alignment-truth stop rule so the branch keeps tolerated variation, pair-breaking misalignment, and the first explanatory split separate before widening into causality claims; the branch should also now preserve a small but important rule that watched-object and query scope are part of **truth selection**, not just tooling efficiency, because oversized watched objects can return real-yet-misleading upstream edges that do not answer the operator’s actual causal question, and it should now preserve a second thinner reminder for watchpoint/query-heavy work: a watchpoint hit is often a **location fact** before it is an object-lifetime fact, so allocation/copy/rebinding/reuse cases should keep `same address != same object != same consequence-bearing incarnation` visible before narrating a returned write as the first-bad-write answer
  - iOS practical workflows, now including a clearer callback/block stop rule around freezing the first runtime-backed contract instead of reopening broad owner search too early, plus a dedicated Swift-concurrency continuation seam when callback truth is already good enough but the first continuation-owned consequence boundary still hides the real consumer, and now a sharper reminder that some Swift-heavy cases still hide the first behavior-bearing proof object one hop later at a MainActor-isolated view-model / coordinator / UI-state handoff rather than at raw callback or resume time; the branch should also preserve exact-once continuation discipline as practical branch memory, so callback visibility, continuation creation/storage, actual resume, missing-resume leak/suspend, double-resume misuse, resumed reducer truth, actor-hop / executor-handoff truth when relevant, and MainActor-side consumer truth do not collapse into one vague async-drift story, it should now also preserve a narrower stream-shaped stop rule so `AsyncStream` / `AsyncThrowingStream` construction, buffering policy, first `yield(...)` visibility, finish/termination/cancellation truth, and the first iterator-side or later MainActor-side durable consumer are not flattened into one generic async event; within that stream-shaped seam, preserve the smaller ladder `yielded != enqueued != consumed != terminated != durable-effect` so producer traffic, accepted buffering, consumer wakeup, stream-lifetime end-state, and later behavior-bearing consequence stay distinct in compare pairs. The branch should now also preserve a thinner Apple-platform service-handoff seam so client-side `NSXPCConnection` / Mach-service truth, listener acceptance, exported-object method entry, reply/error truth, explicit invalidation truth, reconnection-after-restart truth, and the first durable service-owned consequence are not flattened into one vague “XPC happened” claim; within that seam, the branch should now also remember that framework-visible interruption and invalidation are worth separating when possible, with invalidation treated as a stronger end-of-life boundary than ordinary interruption-style drift, because a later healthy-looking connection is weaker than proving the same exported-object method or later reducer actually ran again
  - protected-runtime deobfuscation ladders, now including a thinner opaque-predicate / computed-next-state bridge between broad VM anchoring and broader flattened state-edge reduction, with stronger dispatch-family-aware recovery memory around helper outputs, table indexes, copied-code normalization, and now also encoded-branch / obfuscated-next variants; preserve the sharper operator reminder that **dispatcher recognized != next-state carrier recovered != trustworthy successor relation != full deobfuscation required**, so opaque cleanup, symbolic execution, or IL repair are treated as narrow successor-extraction / noise-reduction aids when the real missing object is still one trustworthy edge, plus a sharper packed-startup stop rule for Windows/native cases where raw PE entry, startup-owned pre-entry truth, raw post-unpack transfer, and the first payload-bearing post-startup handoff may all be different proof objects when TLS callbacks or CRT/runtime startup still dominate, and now also a loader-side image-repair caution so early tampered header/IAT views are kept separate from later post-repair execution truth when remap/repair may occur before entry
- protected-runtime exception-owned control-transfer work, now sharpened around dispatcher-side landing, dynamic-function-table ownership, trap-family compare pairs, and a clearer landing-truth vs resume-truth split so the branch does not stall at merely proving VEH/SEH existence or at naming dispatcher infrastructure before one behavior-bearing resumed target is actually preserved; it should now also preserve a thinner middle layer of **range ownership truth** for Windows dynamic-unwind cases, so callback/growable-table registration is frozen as one installed owning range before the branch jumps from vague dispatcher naming straight to vague later-resume claims, and it should preserve the still-sharper operator split **registered != covering this PC != lookup hit != resumed consequence** so API presence alone is not overread as proof that the current branch is really owned by that dynamic unwind region; the branch should now also preserve a second thinner guard-page / fault-owned stop rule, **guard configured != first fault != re-armed mechanism != resumed consequence**, so one `PAGE_GUARD` hit, single-step landing, or signal delivery does not silently collapse into proof of sustained mechanism ownership or behavior truth
- protected-runtime kernel-callback telemetry work, now sharpened around the narrower stop rule that **registration truth**, **callback firing truth**, **emitted telemetry truth**, and **enforcement-consumer truth** are often different proof objects; in handle-filter cases the downgraded rights-bearing object may already be the first truthful consumer, while anti-cheat-style queue / shared-record / IOCTL / service handoff cases should not stop at callback-family inventory alone
- native desktop/server practical workflows, now including a thinner GUI continuation around per-window subclass identity, `DefSubclassProc(...)` chain-presence versus actual helper-owner proof, Qt event-filter fate versus later handler/slot ownership, the smaller split **signal found != queued truth != first behavior-changing consumer**, a sharper Qt signal-slot delivery rule **connected != direct != queued != delivered != consumed** so recovered `AutoConnection` edges do not silently become queued-or-consumer proof without receiver-affinity and receiver-loop-delivery truth, and the rule that application-global GUI hooks are usually reduction boundaries rather than automatic first-consumer proof; the Linux watcher side of the native branch should now also preserve a thinner delivery rule that **watch registration != relevant event occurred != returned event record fully explains history != first event-owned consumer proved**, so coalesced inotify records, rename-cookie pairing, `IN_Q_OVERFLOW`, and fanotify permission-response ownership are not flattened into one vague “watcher saw it” claim
...
- inside the protocol/service-contract seam, also preserve the stop rule **described != registered != reachable**, so embedded descriptors, reflection-visible rosters, and live callable surfaces do not silently collapse into one proof claim
...
- inside that thinner GUI continuation, preserve one extra practical reminder: global framework hooks are often only reduction boundaries, so prefer one per-window, per-receiver-thread, or per-responder first consumer over stopping at shared subclass wrappers, signal emission, `installEventFilter(...)` / `eventFilter(...)` visibility, or `NSApplication sendEvent:` visibility; in Qt-heavy cases specifically, freeze receiver thread affinity before narrating `AutoConnection`, keep connection truth separate from delivery truth, and remember that even explicit same-thread `QueuedConnection` is still delayed rather than immediate
...
- completion-port / thread-pool first-consumer continuation (`native-completion-port-and-thread-pool-first-consumer-workflow-note`), which acts as the thinner native continuation once the broad async bottleneck has already been reduced into posted work, completion packets, helper-owned thread-pool callbacks, or queue-dequeue ownership and the remaining question is specifically which delivered work item first changes behavior rather than which callback family exists at all; keep explicit stop rules there around posted-control-packet versus I/O-owned-packet truth, `completion key` versus `OVERLAPPED*` ownership roles, the smaller split **explicit event signaled != IOCP packet dequeued != TP_IO callback delivered**, `CreateThreadpoolIo` versus **per-operation** `StartThreadpoolIo` truth, FALSE-with-non-NULL-`lpOverlapped` delivery realism, immediate-success gaps under `FILE_SKIP_COMPLETION_PORT_ON_SUCCESS`, and libuv worker-side versus loop-thread completion ownership
- window-creation / subclass first-consumer continuation (`native-window-creation-and-subclass-first-consumer-workflow-note`), which acts as a thinner Windows-heavy native/GUI continuation once the broad callback bottleneck has already narrowed specifically into Win32 class registration, concrete HWND creation, create-time message flow, or subclass installation and the remaining question is which per-instance procedure/subclass chain first changes behavior rather than merely proving class metadata or a nominal WndProc
- inotify/fanotify first-event-consumer continuation (`native-inotify-fanotify-first-event-consumer-workflow-note`), which acts as a thinner Linux-native continuation once the broad async bottleneck has already narrowed specifically into filesystem watch registration, concrete event-record delivery, or watch-mask/object-scope handling and the remaining question is which first event-owned consumer really changes behavior rather than merely proving watcher setup; preserve the sharper delivery-side rule that returned records may already be shaped by coalescing, rename-cookie pairing, overflow, or permission-response semantics, so `watch registration != relevant event occurred != returned event record fully explains history != first event-owned consumer proved`
- wait-object / threadpool-wait first-consumer continuation (`native-wait-object-and-threadpool-wait-first-consumer-workflow-note`), which acts as a thinner Windows-heavy native continuation once the broad async bottleneck has already narrowed specifically into registered waits, threadpool waits, or wait-object signaling/timeout ownership and the remaining question is whether one registered wait actually became active because of one signal or timeout and which first wait-owned consumer really changed behavior rather than merely proving registration
- timer-queue / threadpool-timer first-consumer continuation (`native-timer-queue-and-threadpool-timer-first-consumer-workflow-note`), which acts as a thinner Windows-heavy native continuation once the broad async bottleneck has already narrowed specifically into timer queues, legacy timer callbacks, or threadpool timer delivery and the remaining question is whether one timer object was still armed, whether one callback was actually queued or raced past cancellation, and which first timer-owned consumer really changed behavior rather than merely proving registration
- APC / alertable-wait first-consumer continuation (`native-apc-alertable-wait-first-consumer-workflow-note`), which acts as a thinner Windows-heavy native continuation once the broad async bottleneck has already been reduced into ordinary user-mode APC delivery and the remaining question is specifically which queued APC actually becomes behaviorally real under one alertable wait or `NtTestAlert` boundary; keep explicit stop rules there around queued-vs-delivered truth, cross-process queueing weakness, and not stopping at `KiUserApcDispatcher` when the real owner lives in the callback body or one immediate downstream consumer
...
- WebSocket upgrade -> first framed-consumer continuation (`protocol-websocket-upgrade-to-first-framed-consumer-workflow-note`), which acts as a thinner protocol continuation once the opening handshake is already good enough and the real bottleneck is identifying which first frame-owned parser/dispatcher/consumer becomes behaviorally real after the Upgrade boundary
...
- pending-request generation / epoch / slot-reuse realism (`protocol-pending-request-generation-epoch-and-slot-reuse-workflow-note`), which acts as the thinner practical continuation when broad owner-match is already good enough but the remaining failure is now live-vs-stale ownership realism: timeout/cancel cleanup, reconnect, wrap/phase drift, or slot/tag reuse deciding whether the same visible identifier still names the current request; keep explicit branch memory that queue arrival, callback-path success, stable slot/index visibility, and other outer-token matches are all weaker than current-owner proof, with useful cross-family reminders now preserved from virtio wrap-owned used-entry truth, NVMe phase-owned completion truth, and io_uring `user_data` vs current waiter/request-context truth
...
- then prove one ownership, consequence, acceptance, pending-owner lifetime contract when broad owner-match is already good enough, output, or hardware-side edge instead of widening protocol narration forever
...
- service failure-action / timeout-abuse continuation (`malware-service-failure-action-and-timeout-abuse-workflow-note`), which acts as the practical thinner-family continuation when the broad persistence step has already narrowed specifically to Windows service persistence but the remaining uncertainty is the narrower contract shape: valid `ServiceMain` ownership, `svchost` / `ServiceDll` hosting, SCM timeout abuse for a non-service executable, or failure-action / restart-owned persistence; this leaf should now also preserve recovery presence, failure-count/reset-window state, exact recovery-slot selection, and queued-restart truth separately so restart-loop or run-command effects are not overattributed to generic crash handling and a later manual/analyst-visible restart is not automatically mistaken for a fresh recovery-selection event; the branch should explicitly leave broad service-install inventory and generic crash narration here once one contract-owned chain is already good enough and the real bottleneck becomes downstream malware-stage proof, evidence packaging, or another narrower continuation
- WMI permanent-subscription consumer proof (`malware-wmi-permanent-event-subscription-consumer-proof-workflow-note`), which acts as the practical thinner-family continuation when the broad persistence step has already narrowed specifically to WMI permanent event subscriptions and the analyst still needs to reduce one `__EventFilter` plus logical consumer plus `__FilterToConsumerBinding` chain into one durable execution-relevant proof object; this continuation should now also preserve the sharper split **defined filter != live bound chain != consumer execution form != final malware effect** so `CommandLineEventConsumer` and `ActiveScriptEventConsumer` cases do not collapse into one generic `WmiPrvSe.exe` story, and MOF/setup provenance stays earlier than later triggered execution truth; the branch should explicitly leave broad WMI artifact inventory here once one bound trigger/action chain and one consumer-shaped later effect are already good enough and the real bottleneck becomes downstream malware-stage proof, evidence packaging, or another narrower continuation
- COM / CLSID hijack consumer proof (`malware-com-clsid-hijack-consumer-proof-workflow-note`), which acts as the practical thinner-family continuation when the broad persistence step has already narrowed specifically to COM / CLSID hijacking and the analyst still needs to reduce one trigger process plus one class-resolution winner plus one later host-process load/launch effect into a durable execution-relevant proof object; this branch should now also preserve `registered != loadable != stable != consumed`, plus `activatable != naturally-consumed`, so a written HKCU override or analyst-driven activation path is not overread as the same thing as one stable recurring consumer-owned consequence; the branch should explicitly leave broad CLSID inventory and registry-only narration here once one trigger-process / resolution / consequence chain is already good enough and the real bottleneck becomes downstream malware-stage proof, evidence packaging, or another narrower continuation
...
- TLS-callback-to-first-payload-consumer continuation (`tls-callback-to-first-payload-consumer-workflow-note`), which acts as a thinner Windows/native packed-startup continuation once the analyst already knows the ambiguity is specifically TLS-callback-shaped and the remaining bottleneck is proving whether one callback merely owns startup normalization or actually hands off into the first payload-bearing consumer
...
- workflow/state recovery (`browser-side-risk-control-and-captcha-workflows`, `browser-fingerprint-and-state-dependent-token-generation`, `browser-service-worker-fetch-consumer-and-cache-ownership-workflow-note`, `reese84-and-utmvc-workflow-note`, `datadome-geetest-kasada-workflow-note`, `datadome-cookie-challenge-workflow-note`, `geetest-v4-w-parameter-and-validate-workflow-note`, `slider-captcha-state-capture-and-trace-comparison-workflow-note`, and `cloudflare-turnstile-widget-lifecycle-workflow-note`), now also preserving the thinner browser-service-worker rule that registration is weaker than current control, which is weaker than one relevant `fetch`/`respondWith` branch, which is weaker than cache/network ownership and later visible outcome truth
...
- pending-request generation / epoch / slot-reuse realism (`protocol-pending-request-generation-epoch-and-slot-reuse-workflow-note`), which acts as the thinner practical continuation once broad owner-match is already good enough but late replies, timeout/cancel cleanup, reconnect, phase/wrap mismatch, or slot/tag reuse still decide whether the same visible identifier names the current live request or only a stale retired owner; the branch should explicitly leave broad pending-owner discussion once the real unknown is this narrower liveness contract rather than basic owner-match itself, and it should now also preserve the fresher cross-family rule that visible used-index movement, same-slot-after-wrap visibility, or matching `user_data` can all still be weaker than current live-owner truth
...
- descriptor ownership-transfer / completion-visibility proof (`descriptor-ownership-transfer-and-completion-visibility-workflow-note`), which acts as the practical entry note when one descriptor, ring slot, or completion record is already visible enough to track and the remaining uncertainty is the narrower contract: when ownership changes, when the other side may trust the record, whether the case is really coherent shared descriptor memory or streaming/non-coherent DMA-backed visibility, whether notify/doorbell edges are only publish hints or still weaker than actual device observation under posted-MMIO rules, what cache/order or explicit CPU/device trust-transfer boundary matters, and what reclaim or slot reuse proves durable completion; the branch should now preserve the sharper DMA-trust rule `descriptor or completion bytes visible != peer may already trust them != ownership transfer is complete != durable completion consequence proved`, and it should explicitly leave broad ownership/visibility continuation work here once one trust-and-reclaim contract is already good enough and the real bottleneck becomes narrower MMIO effect proof, later ISR/deferred consequence proof, or model realism rather than continued shared-ring narration
- USB URB completion and first-consumer proof (`usb-urb-completion-and-first-consumer-workflow-note`), which acts as the practical entry note when USB transfer submission or usbmon completion visibility is already obvious but the missing object is still which callback/parser/router first gives the completed transfer behavioral meaning; the branch should preserve the sharper rule `transfer exists != relevant completion truth != first completion consumer proved != later visible consequence truth`
...
- hybrid explicit-bridge payload recovery (`webview-native-bridge-payload-recovery-workflow-note`), which acts as the practical entry note when the page/native seam is clearly bridge-shaped but the missing object is still the bridge family, the current frame/world/context truth, or the first native consumer that actually interprets the payload; the branch should now preserve the sharper stop rule `bridge object/handler exists != current frame/world/context used it != visible bridge call or message proved first native consumer truth != payload ownership by the decisive native path`
- iOS URL-loading interception and first-consumer proof (`ios-url-loading-interception-and-first-consumer-workflow-note`), which acts as the practical entry note when a custom `URLProtocol`/`NSURLProtocol`, `protocolClasses` configuration, or `WKURLSchemeHandler` is already visible but the missing object is still current session/webview ownership, interception selection, or the first callback that actually owns the request/response consequence; the branch should preserve the sharper rule `interception family exists != current session/webview uses it != relevant request was selected by it != first interception consumer proved != later visible request/response consequence truth`
