...
- completion-port / thread-pool first-consumer continuation (`native-completion-port-and-thread-pool-first-consumer-workflow-note`), which acts as the thinner native continuation once the broad async bottleneck has already been reduced into posted work, completion packets, helper-owned thread-pool callbacks, or queue-dequeue ownership and the remaining question is specifically which delivered work item first changes behavior rather than which callback family exists at all; keep explicit stop rules there around posted-control-packet versus I/O-owned-packet truth, `completion key` versus `OVERLAPPED*` ownership roles, `CreateThreadpoolIo` versus per-operation `StartThreadpoolIo` truth, immediate-success gaps under `FILE_SKIP_COMPLETION_PORT_ON_SUCCESS`, and libuv worker-side versus loop-thread completion ownership
