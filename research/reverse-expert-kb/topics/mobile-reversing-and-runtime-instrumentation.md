...
- for `AsyncStream`-shaped cases specifically, preserve the smaller ladder `yielded != enqueued != consumed != terminated != durable-effect` so producer traffic, accepted buffering, iterator-side consumption, stream-lifetime end-state, and later behavior-bearing consequence do not silently collapse into one vague “async activity happened” claim
