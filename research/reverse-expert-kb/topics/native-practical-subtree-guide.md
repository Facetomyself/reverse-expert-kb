...
- `topics/native-completion-port-and-thread-pool-first-consumer-workflow-note.md` when the async-ownership break has already narrowed specifically into posted work, completion packets, thread-pool callbacks, or queue-dequeue ownership rather than broad callback-plumbing truth
  - preserve extra stop rules there: do not infer dequeue/consumer order from submit order alone in IOCP cases, do not flatten posted control packets into I/O-owned completions, remember Microsoft’s `PostQueuedCompletionStatus` contract that the system does not validate the returned values and that `lpOverlapped` need not be a real `OVERLAPPED`, do not treat `GetQueuedCompletionStatus(FALSE)` with non-NULL `lpOverlapped` as “no delivery happened,” do not collapse IOCP `completion key` family identity into `OVERLAPPED*` owner recovery, preserve the smaller split **explicit event signaled != IOCP packet dequeued != TP_IO callback delivered**, do not assume `CreateThreadpoolIo` alone proves callback delivery without **per-operation** `StartThreadpoolIo` discipline, do not assume `TP_IO` callbacks must fire for immediately successful overlapped operations under `FILE_SKIP_COMPLETION_PORT_ON_SUCCESS`, and do not flatten libuv worker-side `work_cb` ownership into loop-thread `after_work_cb` or `uv_async_send()` ownership
- `topics/native-window-creation-and-subclass-first-consumer-workflow-note.md` when the async/UI ownership break has already narrowed specifically into Win32 class registration, HWND creation, create-time message flow, or subclass installation and the real bottleneck is which per-instance procedure/subclass chain first changes behavior rather than broad GUI message-pump or callback-plumbing truth
  - preserve extra stop rules there: do not flatten class registration into one concrete HWND owner, do not assume `CreateWindowEx` means the class-registered WndProc remains the active instance owner, and do not flatten subclass installation into first behavior-changing consumer truth
- `topics/native-epoll-eventfd-first-consumer-workflow-note.md` when the async-ownership break has already narrowed specifically into Linux `epoll` / `eventfd` / loop wakeup handling and the real bottleneck is which first loop-owned dispatcher/handler/callback gives readiness or wakeup behavioral meaning rather than broad callback-plumbing truth
  - preserve extra stop rules there: do not flatten registration into relevant readiness truth, do not flatten returned readiness or wakeup-fd visibility into delivery/re-arm truth, do not flatten loop wakeup into first loop-owned consumer proof, and do not overread loop-framework identity as later behavior ownership
- `topics/native-io-uring-submission-completion-consumer-workflow-note.md` when the async-ownership break has already narrowed specifically into Linux `io_uring` SQE/CQE ownership and the real bottleneck is one submitted request’s completion identity, cancellation/multishot lifetime, or first app-side dispatcher rather than broad callback-plumbing truth
  - preserve extra stop rules there: do not flatten ring setup into request ownership, do not flatten `io_uring_prep_*` into submitted/in-flight truth, do not infer completion identity from slot/order when `user_data` is the real bridge, do not flatten CQE visibility into dispatcher consumption, and keep **ring setup != SQE prepared != submitted/accepted != completion visible != CQE consumed/routed != request-owned effect** visible
- `topics/native-inotify-fanotify-first-event-consumer-workflow-note.md` when the async-ownership break has already narrowed specifically into Linux filesystem watch registration, concrete inotify/fanotify event delivery, or watch-mask/object-scope handling and the real bottleneck is which first event record becomes behaviorally real rather than broad callback routing or generic FD activity
  - preserve extra stop rules there: do not flatten watch registration into relevant event truth, do not flatten one event record into first event-owned consumer truth, and do not overread watched-path visibility as downstream effect ownership
- `topics/native-wait-object-and-threadpool-wait-first-consumer-workflow-note.md` when the async-ownership break has already narrowed specifically into registered waits, threadpool waits, or wait-object signaling/timeout ownership and the real bottleneck is one wait object’s registration truth, signal-or-timeout truth, or first behavior-changing wait callback consumer rather than broad callback plumbing or IOCP/timer/APC ownership
  - preserve extra stop rules there: do not flatten wait registration into signal-or-timeout truth, do not flatten signaled-object visibility into callback delivery, do not overread unset/cancel paths as proof that no callback was already queued, and do not assume repeated signaling explains repeated callbacks without honest re-registration/flag truth
- `topics/native-timer-queue-and-threadpool-timer-first-consumer-workflow-note.md` when the async-ownership break has already narrowed specifically into timer queues, legacy timer callbacks, or threadpool timer delivery and the real bottleneck is one timer object’s armed-vs-cancelled truth, callback-queued truth, or first behavior-changing consumer rather than broad callback plumbing or IOCP/APC ownership
  - preserve extra stop rules there: do not flatten timer registration into callback queue/delivery truth, do not overread cancellation/unset paths as proof that no callback raced through, and do not flatten periodic timers into one linear callback instance or stable ordering proof
- `topics/native-apc-alertable-wait-first-consumer-workflow-note.md` when the async-ownership break has already narrowed specifically into Windows user-mode APC delivery and the real bottleneck is queued-vs-delivered truth rather than broad callback plumbing or IOCP/thread-pool dequeue ownership
...
- `topics/native-cocoa-responder-chain-and-target-action-first-consumer-workflow-note.md` when the broad GUI continuation has already narrowed specifically into AppKit target/action or responder-chain routing and the remaining bottleneck is whether the first honest boundary is action emission, nil-target resolution, exact receiver truth, or the first durable consumer behind that receiver rather than broad Cocoa event-delivery structure
  - preserve extra stop rules there: do not flatten `sendEvent:` visibility into action ownership, do not flatten `target == nil` into vague receiver folklore, freeze key-window / main-window / first-responder state before narrating receiver truth, use `sendAction:to:from:` / `targetForAction:` as smaller proof surfaces when selector names alone still lie, and keep `selector visible != target resolved != receiver truth != first durable consumer` visible
...
- turning Linux `io_uring` SQE/CQE evidence into one request-owned completion, cancellation/lifetime, and first dispatcher-consumer proof
...
- GUI message-pump / subclass, Qt signal-slot ownership, and macOS Cocoa responder/target-action ownership now have narrower continuation pages, but broader Cocoa XPC/dispatch variants and other OS-specific service/daemon splits are still lightly integrated
