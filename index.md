...
- inside that thinner GUI continuation, preserve one extra practical reminder: global framework hooks are often only reduction boundaries, so prefer one per-window, per-receiver-thread, or per-responder first consumer over stopping at shared subclass wrappers, signal emission, `installEventFilter(...)` / `eventFilter(...)` visibility, or `NSApplication sendEvent:` visibility
