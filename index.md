...
- once code is readable enough to navigate, the branch usually reduces through six recurring bottlenecks:
  ...
  - callback-registration / event-loop consumer proof, with thinner GUI and Linux-reactor continuations when the bottleneck narrows specifically into Win32 message-pump / subclass, Qt signal-slot / responder ownership, Cocoa/XPC/dispatch delivery, or epoll/eventfd/timerfd/libuv readiness-vs-delivery truth
- inside those thinner continuations, preserve two extra practical reminders: global GUI hooks are often only reduction boundaries, so prefer one per-window, per-receiver-thread, or per-responder first consumer over stopping at shared subclass wrappers, signal emission, `installEventFilter(...)` / `eventFilter(...)` visibility, or `NSApplication sendEvent:` visibility; in Qt-heavy cases specifically, freeze receiver thread affinity before narrating `AutoConnection`, keep connection truth separate from delivery truth, and remember that even explicit same-thread `QueuedConnection` is still delayed rather than immediate; in Linux reactor-heavy cases specifically, keep readiness, `epoll_wait(...)` return, drain/rearm truth, and later callback/consumer truth separate, and remember that libuv `uv_async_send(...)` coalescing makes repeated sends weaker than one-for-one callback proof
