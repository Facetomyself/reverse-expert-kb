...
This branch should now be read as a practical bridge from baseline native synthesis into three recurring operator bottlenecks:
...
- async callback/consumer proof (`native-callback-registration-to-event-loop-consumer-workflow-note`), which acts as the practical entry note when registrations, message pumps, completions, or reactor-loop helpers are already visible but the first consequence-bearing callback delivery or event-loop consumer still has to be proved before the subsystem map becomes behaviorally trustworthy
...
This branch should now be read as a practical bridge from mature protected/deobfuscation synthesis into four recurring operator bottlenecks:
...
- iOS practical ladder: environment-gate diagnosis (`ios-packaging-jailbreak-and-runtime-gate-workflow-note`) first, then post-gate owner localization (`ios-objc-swift-native-owner-localization-workflow-note`), then callback/result-to-policy localization (`ios-result-callback-to-policy-state-workflow-note`) once visible result material already exists; together these notes separate broad setup/gate uncertainty, consequence ownership across ObjC / Swift / native boundaries, and the narrower proof of the first behavior-changing local policy state
