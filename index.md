...
- **now materially established and worth preserving canonically**:
  ...
  - protected-runtime deobfuscation ladders, now including a thinner opaque-predicate / computed-next-state bridge between broad VM anchoring and broader flattened state-edge reduction, with stronger dispatch-family-aware recovery memory around helper outputs, table indexes, and copied-code normalization, plus a sharper packed-startup stop rule for Windows/native cases where TLS callbacks or CRT/runtime startup mean the first raw post-unpack transfer is still earlier than the first payload-bearing post-startup handoff, and now also a loader-side image-repair caution so early tampered header/IAT views are kept separate from later post-repair execution truth when remap/repair may occur before entry
