...
- external debugger/watchpoint material reinforces that ordinary watchpoints are often fundamentally tied to an expression or memory location, not to a high-level semantic object lifetime; that is useful precisely because it explains why “the watchpoint fired at the same address” can still be weaker than the operator’s real object-identity question
- GDB-style dynamic-memory discussions are especially useful here because they make the rebind problem explicit: if the object is reallocated, copied, or newly assigned, keeping the old address watchpoint alive can silently track the wrong storage rather than the current owner that matters
- if the answer is no, freeze one stronger identity anchor first (allocation/lifetime event, owner pointer, container slot, generation counter, compare-aligned rebinding point, or object-bearing handle) before overreading the returned writes as the first bad write for the operator’s real question
