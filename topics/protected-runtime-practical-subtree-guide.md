...
- **recover** one handler-owned transfer boundary when traps, faults, or signal delivery hide the real branch, while keeping the thinner stop rule that one guard/fault hit is usually weaker than one re-armed mechanism plus one resumed consequence
...
- for Windows dynamic-unwind cases, preserve one extra middle layer of **range ownership truth** so the branch does not collapse callback/growable-table ownership into either vague dispatcher naming or vague later-resume claims; when `RtlLookupFunctionEntry` keeps making sense only after runtime-installed tables are considered, freeze one installed range first, then one later resume/consequence edge
- preserve the thinner four-way split inside that seam whenever possible: **registered != covering this PC != lookup hit != resumed consequence**; callback/growable-table API presence is weaker than proving that the current PC falls inside the owned region, and that is still weaker than one real unwind-lookup hit plus one behavior-bearing resumed edge
