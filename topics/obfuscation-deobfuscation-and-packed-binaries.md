...
Use `topics/protected-runtime-practical-subtree-guide.md` as the branch entry surface when the case is clearly protected-runtime or deobfuscation shaped, but the current bottleneck still needs to be classified as observation-topology failure, trace/dispatcher churn, packed/bootstrap handoff, string-decryption provenance, artifact-consumer proof, runtime-artifact / initialization-obligation recovery, or integrity/tamper consequence proof before choosing a narrower workflow note.
...
Use `topics/packed-stub-to-oep-and-first-real-module-workflow-note.md` when shelling, packing, or staged bootstrap is already visible and the next bottleneck is not broad packer recognition but proving one trustworthy OEP-like handoff plus one downstream ordinary-code anchor that yields a reusable post-unpack dump or static target; leave broad packed-startup work there once that handoff is already good enough and the real bottleneck becomes post-unpack semantic-anchor work, artifact-consumer proof, or runtime-artifact / initialization-obligation recovery. If the ambiguity has already narrowed specifically to TLS-callback slot listing, replay, replay reason, startup-owned work, and the first payload consumer, continue into `topics/tls-callback-to-first-payload-consumer-workflow-note.md` rather than keeping that thinner ladder implicit inside the broader packed-startup note.

Use `topics/string-decryption-origin-to-artifact-provenance-workflow-note.md` when encoded / encrypted / stack-built strings are visible or suspected and one decoder, transform slice, or emulator output exists, but plaintext output still lacks trustworthy provenance. Keep decoder candidate, callsite or slice selection, origin bytes / stack construction, key / IV / constants, decoded-byte validation, and consumer-ready metadata separate before treating recovered strings as durable evidence or handing them to artifact-consumer proof.
