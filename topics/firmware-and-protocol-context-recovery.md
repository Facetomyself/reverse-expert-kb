...
- preserve a second sharper parent-page reminder for USB-transfer cases: `transfer exists != relevant completion truth != first completion consumer proved != later visible consequence truth`; visible `usb_submit_urb(...)`, libusb async submission, or usbmon completion should not silently replace the smaller callback/parser/router proof object that first gives the completed transfer behavioral meaning
