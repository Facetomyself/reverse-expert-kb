...
- `topics/native-callback-registration-to-event-loop-consumer-workflow-note.md` when the async-ownership break has already narrowed into callback, reactor, or event-loop delivery ownership and the real bottleneck is one queue/ready/delivery/consumer boundary rather than broad callback-plumbing truth
  - preserve extra stop rules there: do not flatten Linux reactor readiness into consumer proof; keep **registered != ready != returned != drained/rearmed != callback-delivered != consumed** visible so `epoll_wait(...)`, `eventfd` / `timerfd` readability, or `uv_async_send(...)` visibility do not silently collapse into one claimed consumer truth
